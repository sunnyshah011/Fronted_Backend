...
const Profile = () => {
  ...
  // ❌ Removed `cities` state (no need since city will be a text input)
  ...
  const [formData, setFormData] = useState({
    ...
    province: "",
    district: "",
    city: "", // ✅ keep this for user-typed city value
    streetAddress: "",
  });
  ...
  // ❌ Removed fetchCities() since city is now a free input
  /*
  ...
  */
  ...
  useEffect(() => {
    if (!isFirstTime) {
      const updatedForm = {
        ...
        city: address.city || "", // keep existing city text
  ...
  // ✅ Handle input change (cleaned + cascade reset logic)
  const handleChange = (e) => {
    ...
    if (name === "province") {
      ...
      updatedForm.city = ""; // ✅ reset city if province changes
      setDistricts([]);
      fetchDistricts(cleanedValue);
    }

    if (name === "district") {
      updatedForm.city = ""; // ✅ reset city if district changes
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      city: formData.city, // ✅ user-input city
  ...
  return (
    <div className="px-4 mt-5 h-full pb-10">
      ...
      >
        ...
        {/* Province and District as dropdowns */}
        {[
        ...
        {/* ✅ City changed from dropdown → input field */}
        <div className="mb-4">
          <label className="block mb-1 font-medium text-gray-700">
            City/Municipality *
          </label>
